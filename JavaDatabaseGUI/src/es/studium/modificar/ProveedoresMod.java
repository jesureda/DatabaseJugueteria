...
public class ProveedoresMod extends JFrame implements ItemListener, ActionListener {
	...
	JLabel label1 = new JLabel("Nombre Empresa: ", JLabel.CENTER);
	JLabel label2 = new JLabel("Dirección: ", JLabel.CENTER);
	JLabel label3 = new JLabel("Teléfono: ", JLabel.CENTER);
	JLabel label4 = new JLabel("Cif: ", JLabel.CENTER);
	...
	public ProveedoresMod() {
		...
		dlgMod.setLayout(new GridLayout(6, 1));
	...
	public void itemStateChanged(ItemEvent ie) {
		...
		try {
			statement = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
			rs = statement.executeQuery("SELECT * FROM Proveedores WHERE idProveedor=" + idParaModificar);
			rs.next();
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (btnSi.equals(ae.getSource())) {
			try {
				statement = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
				statement.executeUpdate("UPDATE Proveedores SET nEmpresa='" + text1.getText() + "' , direccion='"
						+ text2.getText() + "' , telefono=" + text3.getText() + " , cif='" + text4.getText()
						+ "' WHERE idProveedor=" + idParaModificar);
				dlgMod.setVisible(false);
				diaSeguro.setVisible(false);
				new DiaCorrecto();
				statement.close();

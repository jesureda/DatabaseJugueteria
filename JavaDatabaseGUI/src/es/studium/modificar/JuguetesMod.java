...
public class JuguetesMod extends JFrame implements ItemListener, ActionListener {
	...
	public JuguetesMod() {
		...
		dlgMod.setSize(500, 200);
		dlgMod.setLayout(new GridLayout(8, 1));
	...
	public void itemStateChanged(ItemEvent ie) {
		...
		if (dlgMod.isVisible() == false) {
			try {
				statement = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
				rs = statement.executeQuery("SELECT * FROM Juguetes WHERE idJuguete=" + idParaModificar);
				rs.next();
				...
				String FK = Integer.toString(rs.getInt(7));
				rs = statement.executeQuery("SELECT * FROM Proveedores WHERE idProveedor=" + FK);
				rs.next();
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (btnSi.equals(ae.getSource())) {
			...
			try {
				statement = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
				statement.executeUpdate("UPDATE Juguetes SET nombre='" + text1.getText() + "' , descripcion='"
						+ text2.getText() + "' , precio=" + text3.getText() + " , cantidad=" + text4.getText()
						+ " , edadRecomendada='" + text5.getText() + "' , idProveedorFK=" + fk + " WHERE idJuguete="
						+ idParaModificar);
				dlgMod.setVisible(false);
				diaSeguro.setVisible(false);
				listaFK.select(0);

...
public class PedidosJuguetesMod extends JFrame implements ActionListener, ItemListener {
	...
	JLabel label1 = new JLabel("idPedidoFK: ", JLabel.CENTER);
	...
	public PedidosJuguetesMod() {
		setTitle("Modificar PedidosJuguetes");
		...
		try {
			...
			rs = statement.executeQuery("SELECT * FROM PedidosJuguetes");
			while (rs.next()) {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (btnSi.equals(ae.getSource())) {
			...
			try {
				statement = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
				statement.executeUpdate("UPDATE PedidosJuguetes SET idPedidoFK=" + FK + ",idJugueteFK=" + FK2
						+ " WHERE idPedidoFK=" + idParaModificar + " AND idJugueteFK=" + idParaModificar2);
				dlgMod.setVisible(false);
				diaSeguro.setVisible(false);
				new DiaCorrecto();
				statement.close();

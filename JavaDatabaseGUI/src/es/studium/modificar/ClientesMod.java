...
public class ClientesMod extends JFrame implements ItemListener, ActionListener {
	...
	public void itemStateChanged(ItemEvent ie) {
		...
		try {
			statement = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
			rs = statement.executeQuery("SELECT * FROM Clientes WHERE idCliente=" + idParaModificar);
			rs.next();
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (btnSi.equals(ae.getSource())) {
			try {
				statement = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
				statement.executeUpdate("UPDATE Clientes SET nombre='" + text1.getText() + "' , apellidos='"
						+ text2.getText() + "' , dni='" + text3.getText() + "' , telefono=" + text4.getText()
						+ " WHERE idCliente=" + idParaModificar);
				dlgMod.setVisible(false);
				diaSeguro.setVisible(false);
				new DiaCorrecto();
				statement.close();

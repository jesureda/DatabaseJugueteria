...
public class VentasMod extends JFrame implements ActionListener, ItemListener {
	...
	public void itemStateChanged(ItemEvent ie) {
		...
		if (dlgMod.isVisible() == false) {
			try {
				...
				text2.setText(Double.toString(rs.getDouble(3)));
				String FK = Integer.toString(rs.getInt(4));
				rs = statement.executeQuery("SELECT * FROM Empleados WHERE idEmpleado=" + FK);
				rs.next();
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (btnSi.equals(ae.getSource())) {
			...
			try {
				statement = connection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
				statement.executeUpdate("UPDATE Ventas SET fechaEmision='" + text1.getText() + "' ,  importe="
						+ text2.getText() + " , idEmpleadoFK=" + fk + " WHERE idVenta=" + idParaModificar);
				dlgMod.setVisible(false);
				diaSeguro.setVisible(false);
				listaFK.select(0);
				new DiaCorrecto();

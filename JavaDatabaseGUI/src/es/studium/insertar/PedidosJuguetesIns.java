...
public class PedidosJuguetesIns extends JFrame implements ActionListener, ItemListener {
	...
	public PedidosJuguetesIns() {
		// Ventana
		setTitle("PedidosJuguetes");
		setResizable(false);
		setSize(300, 120);
		...
		// Rellena el choice de la FK con los Pedidos
		try {
		...
		if (btnSi.equals(ae.getSource())) {
			try {
				...
				statement.executeUpdate(
						"INSERT INTO PedidosJuguetes (idPedidoFK, idJugueteFK) VALUES " + "(" + FK + "," + FK2 + ")");
				lista.select(0);
				lista2.select(0);
				diaSeguro.setVisible(false);
				new DiaCorrecto();
